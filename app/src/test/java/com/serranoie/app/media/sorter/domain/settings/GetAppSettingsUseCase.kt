...
class SorterMediaUseCaseTest {
	...
	@Before fun setUp() { useCase = SorterMediaUseCase(mockRepository) }
	@After fun tearDown() = clearAllMocks()

	@Test
	fun invoke_success_filtersAndSorts() = runTest {
		val valid = MediaFile(mockk(), "image", "jpg", "file1", "dir", 256L, LocalDate.now(), mockk(), 100L)
		val invalid = MediaFile(mockk(), "image", "jpg", "", "dir", 0L, LocalDate.now(), mockk(), 50L)
